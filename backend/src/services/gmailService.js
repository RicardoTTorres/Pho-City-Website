...
const clientCache = new Map(); // email -> GmailClient or ImapClient

export async function getEmailClient({email = undefined, send = true, inbox = true} = {}) {
    if (!email) {
    ...
    let {authenticated: gmailAuthenticated, registered: gmailRegistered} = await client.checkAuth({send, inbox});
    ...
    let {authenticated: imapAuthenticated, registered: imapRegistered} = await client.checkAuth({send, inbox});
    ...
    return {client: undefined, authenticated: false, registered: gmailRegistered || imapRegistered, email};
}
...
export class ImapClient {
    ...
    async checkAuth({send = true, inbox = true} = {}) {
        ...
        if (inbox) {
            try {
                ...
                if (!this.client.usable) return {authenticated: false, registered: true};
            } catch (err) {
                return {authenticated: false, registered: true};
            }
        }
    ...
    async fetchThreads({pageToken = undefined, maxResults = 20} = {}) {
        ...
        try {
            ...
            const threads = await Promise.all(threadIds.map(id =>
                this.fetchThread(id, {preview: true, allContent: false, recentContent: true})
            ));
    ...
    async downloadMessageContent(uid, part, maxBytes = undefined) {
        const download = await this.client.download(
            `${uid}`,
            part,
            {uid: true, maxBytes: part === undefined ? undefined : maxBytes}
        );
    ...
    async fetchThread(id, {historyId = undefined, preview = false, forceRefresh = false, allContent = true, recentContent = false } = {}) {
        await this.connectInbox();
        ...
        try {
            ...
            const messages = await Promise.all(rawMessages.map(async (message, index) => {
                const parsed = parseImapMessage(message, this.email);
            
                let body;
                let snippet;
                if (allContent || (recentContent && index == rawMessages.length - 1)) {
                    snippet = "";
                    ...
                    if (rows.length == 0 || (rows[0].is_preview && !preview)) {
                        if (preview) {
                            // download snippet
                            const {found, part} = bodyStructureFindPart(message.bodyStructure, false);
                            if (found) {
                                snippet = await this.downloadMessageContent(message.uid, part, 300);
                                snippet = snippet.split(/On [A-z]{3}, [A-z]{3} [0-9]{1,2}, [0-9]{1,4} at [0-9]{1,2}:[0-9]{2}.(A|P)M .*? wrote:/)[0].trimEnd()
                                if (parsed.contactForm && snippet.startsWith("Message from ")) {
                                    snippet = snippet.replace(/Message from .*? \(.*?\) via Pho City Website Contact Form:/, "");
                                }
                            }

                        } else {
                            // download full body
                            const {found, part, type} = bodyStructureFindPart(message.bodyStructure, true);
                            body = "";
                            if (found) {
                                ...
                                if (body.startsWith("Message from "))
                                    body = body.replace(/Message from .*? \(.*?\) via Pho City Website Contact Form:/, "");
                                body = body.trim();
                        ...
                        await pool.query(
                            ...
                            `,
                            [this.email, message.emailId, body === undefined, snippet, body]
                        );
    ...
    async reply(threadId, body) {
        const thread = await this.fetchThread(threadId, {preview: true, allContent: false});
        const lastMessage = thread.messages.at(-1);
